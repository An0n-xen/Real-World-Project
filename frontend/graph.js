class PipelineGraph {
    ...
    static STATUS_COLORS = {
        completed: { bg: 'rgba(0, 230, 118, 0.12)', border: '#00e676', text: '#00e676', glow: 'rgba(0, 230, 118, 0.3)' },
        running:   { bg: 'rgba(0, 229, 255, 0.12)', border: '#00e5ff', text: '#00e5ff', glow: 'rgba(0, 229, 255, 0.3)' },
        error:     { bg: 'rgba(255, 82, 116, 0.12)', border: '#ff5274', text: '#ff5274', glow: 'rgba(255, 82, 116, 0.3)' },
        pending:   { bg: 'rgba(90, 103, 133, 0.10)', border: '#5a6785', text: '#5a6785', glow: 'rgba(90, 103, 133, 0.15)' },
    };
    ...
    _drawNode(node, hovered) {
        ...
        ctx.fillStyle = hovered ? 'rgba(14, 22, 48, 0.9)' : colors.bg;
        ctx.beginPath();
        ...
        // Status badge — right-aligned INSIDE the box
        ctx.fillStyle = colors.text;
        ctx.globalAlpha = 0.7;
        ctx.font = '500 9px Inter, sans-serif';
        ctx.textAlign = 'right';
        ctx.fillText(node.status, x + w - 12, y + h / 2);
        ctx.textAlign = 'left';
        ctx.globalAlpha = 1;
    ...
    _showTooltip(e, node) {
        const tooltip = document.getElementById('node-tooltip');
        document.getElementById('tooltip-header').textContent = node.label + ' — ' + node.status;
    ...
    _moveTooltip(e) {
        ...
        if (tx + t.offsetWidth > innerWidth - 10) tx = e.clientX - t.offsetWidth - 10;
        if (ty + t.offsetHeight > innerHeight - 10) ty = e.clientY - t.offsetHeight - 10;
        t.style.left = tx + 'px';
        t.style.top = ty + 'px';
    }
}
...
class FindingsGraph {
    ...
    static CATEGORY_COLORS = {
        diagnosis:   { fill: '#00e676', border: '#00e676', bg: 'rgba(0, 230, 118, 0.15)' },
        evidence:    { fill: '#00e5ff', border: '#00e5ff', bg: 'rgba(0, 229, 255, 0.12)' },
        finding:     { fill: '#00bcd4', border: '#00bcd4', bg: 'rgba(0, 188, 212, 0.10)' },
        condition:   { fill: '#ff5274', border: '#ff5274', bg: 'rgba(255, 82, 116, 0.10)' },
        symptom:     { fill: '#ffab40', border: '#ffab40', bg: 'rgba(255, 171, 64, 0.10)' },
        anatomy:     { fill: '#b388ff', border: '#b388ff', bg: 'rgba(179, 136, 255, 0.10)' },
        measurement: { fill: '#69f0ae', border: '#69f0ae', bg: 'rgba(105, 240, 174, 0.10)' },
    };
    ...
    _showTooltip(e, node, tooltip) {
        ...
        if (node.detail) {
            ...
            html += `<div style="margin-top:4px;color:#8b98b8;font-size:0.7rem;line-height:1.4;">${det}</div>`;
        }
        body.innerHTML = html;
        tooltip.style.display = 'block';
